...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load Data
    try {
        const response = await fetch(`quiz.${lang}.yaml`);
...
function showMainPage() {
    ...
    mainPage.innerHTML = `
        <img src="images/${image}" class="hero-image-original" alt="${title}">
        <h1>${title}</h1>
        <p>${description}</p>
    ...
    question.answers.forEach((answer, index) => {
        answersHtml += `
            <div class="answer-card" onclick="handleAnswer(${index})">
                <img src="images/${answer.image}" class="answer-image" loading="lazy">
...
function showResult() {
    ...
    const closeText = quizData.close_button || "Close";

    const isDark = tg.colorScheme === 'dark';
    let resultColor = result.color || 'var(--button-color)';
    if (isDark && result.color_dark) {
        resultColor = result.color_dark;
    }

    resultPage.innerHTML = `
        <img src="images/${result.image}" class="hero-image" alt="${result.title}">
